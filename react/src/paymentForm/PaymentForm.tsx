...
import {type SecureFieldsClient, type SecureFieldsConfig} from "@purse-eu/web-sdk";

import {
    initSecureFields as moduleInitSecureFields
} from "https://cdn.purse-dev.com/secure-fields/latest/purse.esm.js?module"
import {useEffect, useRef, useState} from "react";
import {TokenizationResultDisplay} from "./TokenizationResultDisplay.tsx";

export const PaymentForm = ({embeddedBrandSelector}: { embeddedBrandSelector?: boolean }) => {
    const [brands, setBrands] = useState<CardBrand[]>([])
    const [selectedBrand, setSelectedBrand] = useState<CardBrand | null>(null)
    const [tokenizationResult, setTokenizationResult] = useState<SubmitResult | null>(null);
    ...
    return (
        <div>
            ...
            {!embeddedBrandSelector &&
                <InlineBrandSelector brands={brands} selectedBrand={selectedBrand} onChange={setSelectedBrand}/>}

            <label htmlFor="cvv" className="block text-sm font-medium text-gray-700 mt-4">
                CVV
            </label>
            <div
                id="cvv"
            ...
            <label htmlFor="expiry-date" className="block text-sm font-medium text-gray-700 mt-4">
                Expiry Date
            </label>
            <div
                id="expiry-date"
            ...
            <label htmlFor="holder-name" className="block text-sm font-medium text-gray-700 mt-4">
                Card Holder Name
            </label>
            <div
                id="holder-name"
            ...
            {tokenizationResult && <TokenizationResultDisplay tokenizationResult={tokenizationResult}/>}
        </div>
    )
}